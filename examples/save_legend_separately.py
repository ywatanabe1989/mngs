...
def save_legend_separately_mngs():
    """Example using MNGS plotting framework."""
    # Create sample data
    x = np.linspace(0, 10, 100)
    data = {
        'sin(x)': np.sin(x),
    ...
    # Save full plot
    fig.save('mngs_plot_with_legend.png')
    ...
    save_legend_from_axis(ax._axis_mpl, 'mngs_legend_only.png')
    
    # Remove legend and save plot without it
    if legend:
        legend.remove()
    fig.save('mngs_plot_without_legend.png')
...
if __name__ == "__main__":
    ...
    print("2. MNGS framework legend extraction")
    save_legend_separately_mngs()
