...
# File: ./examples/mngs_io_example.py
# ----------------------------------------
import os
__FILE__ = (
    "./examples/mngs_io_example.py"
)
...
"""
Functionalities:
  - Demonstrates mngs.io module capabilities
...
mngs.io.makedirs(OUTPUT_DIR, exist_ok=True)

"""Main Script"""
def main():
    print("MNGS I/O Module Demonstration")
    print("=" * 50)
    ...
    for fmt, path in formats.items():
        if fmt == 'npy':
            mngs.io.save(df.values, path)
        else:
            mngs.io.save(df, path)
    ...
    mngs.io.save(large_df, csv_path, index=False)
    ...
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_title('MNGS I/O Example Plot')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # Save plot
    img_path = f"{OUTPUT_DIR}/sample_plot.png"
    mngs.io.save(fig, img_path)
    ...
    mngs.io.save(compressed_data, npz_path)
    ...
    mngs.io.save(nested_data, json_path)
...
providing significant performance improvements for repeated saves."""
    ...
    mngs.io.save(summary, summary_path)
    print(summary)
