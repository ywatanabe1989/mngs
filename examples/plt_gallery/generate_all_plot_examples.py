...
# -*- coding: utf-8 -*-
"""
Generate comprehensive examples for all mngs.plt plot types.
...
3. **Directory separation**: Using extensions in mngs.io.save() path separates file types:
   - mngs.io.save(fig, "gif/plot.gif") creates:
...
plt, CC = mngs.plt.utils.configure_mpl(
    plt,
    fig_size_mm=(160, 120),
    dpi_display=100,
    dpi_save=150,
...
def example_ax_plot():
    ...
    fig, ax = mngs.plt.subplots(figsize=(8, 6), dpi=100)
...
def example_plot_raster():
    """Raster plot example with proper trial/channel positioning using mngs colors."""
    fig, ax = mngs.plt.subplots()
    ...
    # Use mngs custom colors for consistent styling
    colors = [
        mngs.plt.color.str2rgba('blue', alpha=0.8),
        mngs.plt.color.str2rgba('red', alpha=0.8), 
        mngs.plt.color.str2rgba('green', alpha=0.8),
        mngs.plt.color.str2rgba('orange', alpha=0.8)
    ]
...
def example_colormap_colors():
    ...
    colors = mngs.plt.color.get_colors_from_cmap(cmap_name='viridis', n_colors=n_lines)
...
def main():
    ...
    print("Generating mngs.plt examples...")
