...
# File: ./examples/mngs_demo.py

"""
Functionalities:
- Demonstrates core MNGS functionality with a complete scientific workflow
- Shows data generation, processing, analysis, and visualization
- Illustrates best practices for using MNGS modules

Example usage:
$ python ./examples/mngs_demo.py

Input:
- None (generates synthetic data)

Output:
- ./examples/mngs_demo_output/: Directory containing all outputs
...
def process_signals(signals, fs):
    ...
    for sig in signals:
        ...
        filtered = mngs.dsp.filt.bandpass(sig, fs, bands=[[1, 100]])
        processed_signals.append(filtered)
        
        # Extract power spectral density features
        freqs, psd = mngs.dsp.psd(filtered, fs)
...
def visualize_results(signals1, signals2, features1, features2, freqs, CONFIG):
    ...
    fig1, axes = mngs.plt.subplots(2, 2, figsize=(12, 8))
    ...
    for sig in signals1:
        _, psd = mngs.dsp.psd(sig, fs=1000)
    ...
    mngs.io.save("./examples/mngs_demo_output/figures/signal_analysis.png", fig1)
    
    # 2. Statistical comparison plot
    fig2, ax = mngs.plt.subplots(figsize=(10, 6))
    ...
    mngs.io.save("./examples/mngs_demo_output/figures/statistical_comparison.png", fig2)
...
def main():
    # Initialize MNGS with configuration
    CONFIG, sys_out, sys_err, plt, CC = mngs.gen.start(
        sys=sys,
        sdir="./examples/mngs_demo_output/logs",
        verbose=True,
        seed=42
    )
    
    print("=== MNGS Demo Script ===")
    ...
    # Save raw data
    mngs.io.save("./examples/mngs_demo_output/data/raw_signals_cond1.npy", signals1)
    mngs.io.save("./examples/mngs_demo_output/data/raw_signals_cond2.npy", signals2)
    mngs.io.save("./examples/mngs_demo_output/data/time_vector.npy", time_vec)
    ...
    mngs.io.save("./examples/mngs_demo_output/data/processed_signals_cond1.npy", processed1)
    mngs.io.save("./examples/mngs_demo_output/data/processed_signals_cond2.npy", processed2)
    mngs.io.save("./examples/mngs_demo_output/data/features_cond1.csv", features1)
    mngs.io.save("./examples/mngs_demo_output/data/features_cond2.csv", features2)
    ...
    # Save results
    mngs.io.save("./examples/mngs_demo_output/results/statistical_analysis.csv", results_df)
    ...
    report = f"""# MNGS Demo Analysis Report
...
5. Visualized results using MNGS plotting utilities
"""
    
    mngs.io.save("./examples/mngs_demo_output/ANALYSIS_REPORT.md", report)
    
    print("\n=== Demo Complete ===")
    print(f"All outputs saved to: ./examples/mngs_demo_output/")
    print("Check ANALYSIS_REPORT.md for detailed results")
    
    # Close MNGS (saves logs)
    mngs.gen.close(CONFIG)

if __name__ == "__main__":
    # Run from project root: python ./examples/mngs_demo.py
    main()