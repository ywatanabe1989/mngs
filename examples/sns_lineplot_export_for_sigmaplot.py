...
def plot_and_export_with_mngs(df):
    """Create plot with MNGS and export the data."""
    # Create MNGS figure
    ...
    # Export the plotted data using MNGS export functionality
    exported_df = mplt.export_as_csv(fig.history)
    
    # Save the exported data
    exported_df.to_csv('mngs_exported_plot_data.csv', index=False)
    print(f"Saved MNGS exported data to: mngs_exported_plot_data.csv")
...
if __name__ == "__main__":
    ...
    # Plot and export with MNGS
    print("\n1. Creating plot with MNGS and exporting data...")
    fig, exported_df = plot_and_export_with_mngs(df)
    ...
    print("- mngs_exported_plot_data.csv (MNGS export)")
