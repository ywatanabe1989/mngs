...
def demonstrate_filtering_and_slicing(df):
    ...
    treatment_indices = mngs.pd.find_indi(df, conditions={"group": "treatment"})
    print(f"\nTreatment group indices: {treatment_indices}")

    # Slice DataFrame using conditions
    high_scorers = mngs.pd.slice(df, conditions={"score_1": lambda x: x > 85})
...
def demonstrate_coordinate_transformations():
    ...
    matrix_reconstructed = mngs.pd.from_xyz(xyz_data, x="x", y="y", z="z")
