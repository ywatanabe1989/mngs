...
# File: ./examples/mngs/pd/dataframe_operations.py

__FILE__ = "./examples/mngs/pd/dataframe_operations.py"
...
def demonstrate_basic_operations():
    ...
    # Save for reference
    mngs.io.save(df, "dataframes/original_data.csv")
...
def demonstrate_column_operations(df):
    ...
    mngs.io.save(df_melted, "dataframes/melted_data.csv")
    ...
    df_merged = mngs.pd.merge_columns(
        df_copy,
        'score_1', 'score_2',
        sep=' | ',
    ...
    mngs.io.save(df_merged, "dataframes/merged_columns.csv")
...
def demonstrate_filtering_and_slicing(df):
    ...
    treatment_indices = mngs.pd.find_indi(
        df,
        conditions={'group': 'treatment'}
    )
    print(f"\nTreatment group indices: {treatment_indices}")
    
    # Slice DataFrame using conditions
    high_scorers = mngs.pd.slice(
        df,
        conditions={'score_1': lambda x: x > 85}
    )
    print("\nHigh scorers (score_1 > 85):")
    print(high_scorers)
    mngs.io.save(high_scorers, "dataframes/high_scorers.csv")
    
    # Multiple conditions
    specific_subset = mngs.pd.slice(
        df,
        conditions={
            'group': 'control',
            'age': lambda x: x < 35
...
def demonstrate_type_conversions():
    ...
    df_numeric = mngs.pd.to_numeric(mixed_data.copy())
    print("\nAfter numeric conversion:")
    print(df_numeric)
    print(f"Data types:\n{df_numeric.dtypes}")
    
    # Round numeric columns
    df_rounded = mngs.pd.round(df_numeric, factor=1)
    print("\nAfter rounding to 1 decimal:")
    print(df_rounded)
    
    mngs.io.save(df_rounded, "dataframes/cleaned_numeric.csv")
...
def demonstrate_coordinate_transformations():
    ...
    xyz_data = mngs.pd.to_xyz(correlation_matrix)
    print("\nConverted to XYZ format:")
    print(xyz_data.head(10))
    mngs.io.save(xyz_data, "dataframes/xyz_format.csv")
    
    # Convert back to matrix
    matrix_reconstructed = mngs.pd.from_xyz(
        xyz_data,
        x='x',
        y='y',
        z='z'
...
def demonstrate_missing_value_handling():
    ...
    df_sorted = mngs.pd.sort(data_with_nan, by='time')
    ...
    df_interpolated[['sensor_1', 'sensor_2']] = df_sorted[['sensor_1', 'sensor_2']].interpolate()
    print("\nAfter interpolation:")
    print(df_interpolated)
    
    mngs.io.save(df_interpolated, "dataframes/interpolated_data.csv")
...
def demonstrate_advanced_operations():
    ...
    # Moving calculations using pandas (mngs.pd utilities work seamlessly)
    ...
    pval_col = mngs.pd.find_pval(stats_df)
    print(f"\nFound p-value column: '{pval_col}'")
    
    # Save results
    mngs.io.save(ts_data, "dataframes/timeseries_analysis.csv")
    mngs.io.save(stats_df, "dataframes/statistical_results.csv")
...
def main(args):
    ...
    print("\nMNGS Pandas Utilities Demonstration")
    print("===================================")
...
"""
    ...
    mngs.io.save(report, "reports/pd_examples_summary.md")
    print("\n" + "="*50)
