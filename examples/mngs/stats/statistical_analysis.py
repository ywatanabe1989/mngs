...
# File: ./examples/mngs/stats/statistical_analysis.py
# ----------------------------------------
import os

__FILE__ = "./examples/mngs/stats/statistical_analysis.py"
__DIR__ = os.path.dirname(__FILE__)
# ----------------------------------------

"""
Functionalities:
  - Demonstrates statistical analysis capabilities of mngs.stats
...
Dependencies:
  - scripts: None
  - packages: numpy, pandas, matplotlib, scipy, mngs
...
def demonstrate_p_value_formatting(test_results):
    ...
    results_table["significance"] = [
        mngs.stats.p2stars(p) for p in results_table["p_value"]
    ]
...
"""
    ...
    ax1.set_title(
        f'Group Comparison (p = {test_result["p_value"]:.4f} {mngs.stats.p2stars(test_result["p_value"])})'
    )
    ...
    if test_result["p_value"] < 0.05:
        ...
        ax1.text(
            0.5,
            y_max + 1,
            mngs.stats.p2stars(test_result["p_value"]),
            ha="center",
            va="bottom",
            fontsize=16,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Statistical analysis examples with mngs.stats"
    )
