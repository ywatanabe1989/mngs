...
# File: examples/mngs/ai/machine_learning_workflow.py

__file__ = "examples/mngs/ai/machine_learning_workflow.py"

"""
Machine Learning Workflow with mngs.ai

This example demonstrates:
- Setting up reproducible ML experiments
- Using mngs classifiers and utilities
...
def train_classifiers(X_train, X_val, y_train, y_val):
    ...
    classifiers = {
        'RandomForest': mngs.ai.sklearn.RandomForestClassifier(
            n_estimators=100,
            max_depth=10,
            random_state=42
        ),
        'GradientBoosting': mngs.ai.sklearn.GradientBoostingClassifier(
            n_estimators=100,
            max_depth=5,
            learning_rate=0.1,
            random_state=42
        ),
        'SVM': mngs.ai.sklearn.SVC(
            kernel='rbf',
            probability=True,
            random_state=42
        ),
        'LogisticRegression': mngs.ai.sklearn.LogisticRegression(
            max_iter=1000,
            random_state=42
        )
    }
...
def evaluate_models(results, X_test, y_test):
    ...
    # Classification reporter for detailed metrics
    reporter = mngs.ai.ClassificationReporter(sdir="./output/classification_reports")
    ...
    for name, result in results.items():
        ...
        report = reporter.report(y_test, y_pred_test)
...
def create_visualizations(results, evaluation_results, y_test):
    ...
    # Set up figure
    fig, axes = mngs.plt.subplots(nrows=2, ncols=2, figsize=(12, 10))
    ...
    for i, (name, eval_result) in enumerate(evaluation_results.items()):
        ...
        axes[1, 1].bar(positions + i * bar_width, values, 
    ...
    # Save figure
    mngs.io.save(fig, "./output/ml_analysis.png", dpi=150)
...
"""
    ...
    # Save report
    mngs.io.save(report, "./output/ml_report.md")
...
def main(args):
    ...
    try:
        print("=" * 60)
        print("MNGS Machine Learning Workflow Example")
        print("=" * 60)
        ...
        mngs.io.save(data_info, "./output/dataset_info.json")
        
        # Step 2: Preprocess data
        (X_train, X_val, X_test, 
         y_train, y_val, y_test, scaler) = preprocess_data(X, y)
        ...
        print(f"Best model ({best_model}) achieved {evaluation_results[best_model]['accuracy']:.1%} accuracy")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='MNGS Machine Learning Workflow Example')
