#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Time-stamp: "2025-05-31 Current"
# File: ./examples/mngs/dsp/signal_processing.py

__FILE__ = "./examples/mngs/dsp/signal_processing.py"
...
Dependencies:
  - packages: numpy, matplotlib, mngs
...
def generate_synthetic_signals():
    ...
    theta_3d = mngs.dsp.ensure_3d(theta)
    theta_phase = np.angle(mngs.dsp.hilbert(theta_3d)[0][0, 0, :])
...
def demonstrate_filtering(signal, fs):
    ...
    # Ensure signal is 3D for mngs.dsp functions
    signal_3d = mngs.dsp.ensure_3d(signal)  # Shape: (1, 1, n_samples)
    
    # Bandpass filter for alpha band (8-12 Hz)
    alpha_filtered = mngs.dsp.filt.bandpass(
        signal_3d,
        fs=fs,
        bands=[[8, 12]]  # 2D array for batch dimension
    )
    
    # Lowpass filter (remove high frequencies)
    lowpass_filtered = mngs.dsp.filt.lowpass(
        signal_3d,
        fs=fs,
        cutoffs_hz=np.array([30])  # Must be array
    )
    
    # Highpass filter (remove low frequencies)
    highpass_filtered = mngs.dsp.filt.highpass(
        signal_3d,
        fs=fs,
    ...
    plt.tight_layout()
    mngs.io.save(fig, "plots/filtering_examples.png")
    plt.close()
...
def demonstrate_psd_analysis(signal, fs):
    ...
    # Calculate PSD
    signal_3d = mngs.dsp.ensure_3d(signal)
    psd, freqs = mngs.dsp.psd(signal_3d, fs=fs)  # Returns psd first, then freqs
    ...
    ax.legend()
    mngs.io.save(fig, "plots/psd_analysis.png")
    plt.close()
    ...
    for band, (low, high) in bands.items():
        ...
        band_powers[band] = float(np.mean(psd[0, 0, mask]))  # Convert to Python float for JSON
    
    print("\nBand Powers:")
    for band, power in band_powers.items():
        print(f"  {band}: {power:.2e}")
    
    mngs.io.save(band_powers, "analysis/band_powers.json")
...
def demonstrate_wavelet_analysis(signal, fs):
    ...
    freq_array = freqs[0, 0, :].cpu().numpy() if hasattr(freqs, 'cpu') else freqs[0, 0, :]
    ...
    mngs.io.save(fig, "plots/wavelet_analysis.png")
    plt.close()
...
def demonstrate_hilbert_transform(signal, fs):
    ...
    signal_3d = mngs.dsp.ensure_3d(signal)
    alpha_signal = mngs.dsp.filt.bandpass(signal_3d, fs=fs, bands=[[8, 12]])
    ...
    phase, amplitude = mngs.dsp.hilbert(alpha_signal)
    ...
    plt.tight_layout()
    mngs.io.save(fig, "plots/hilbert_transform.png")
    plt.close()
...
def demonstrate_pac_analysis(signal, fs):
    ...
    for i in range(len(phase_bands)):
        for j in range(len(amp_bands)):
            text = ax.text(j, i, f'{pac_matrix[i, j]:.3f}',
                          ha='center', va='center', color='white' if pac_matrix[i, j] > 0.01 else 'black')
    
    plt.tight_layout()
    mngs.io.save(fig, "plots/pac_analysis.png")
    plt.close()
...
def demonstrate_multi_channel_processing(multi_channel, fs):
    ...
    referenced = mngs.dsp.reference.common_average(multi_3d)
    
    # Calculate channel-wise PSD
    freqs, psds = mngs.dsp.psd(referenced, fs=fs)
    ...
    if psds.ndim == 3:
        ...
        for ch in range(n_channels):
            ax.semilogy(freqs, psds[ch, 0, :], label=f'Channel {ch+1}')
    elif psds.ndim == 2:
        # Shape is (n_channels, n_freqs)
        for ch in range(n_channels):
            ax.semilogy(freqs, psds[ch, :], label=f'Channel {ch+1}')
    else:
        # Single channel
        ax.semilogy(freqs, psds, label='Channel 1')
    ...
    mngs.io.save(fig, "plots/multichannel_psd.png")
    plt.close()
...
def demonstrate_advanced_features(signal, fs):
    ...
    signal_3d = mngs.dsp.ensure_3d(signal)
    resampled = mngs.dsp.resample(signal_3d, src_fs=fs, tgt_fs=new_fs)
    ...
    normalized = mngs.dsp.norm(signal_3d, axis=-1)
    
    # Modulation index calculation
    # Get phase and amplitude for specific bands
    phase_signal = mngs.dsp.filt.bandpass(signal_3d, fs=fs, bands=[[4, 8]])
    amp_signal = mngs.dsp.filt.bandpass(signal_3d, fs=fs, bands=[[30, 80]])
    
    phase, _ = mngs.dsp.hilbert(phase_signal)
    _, amplitude = mngs.dsp.hilbert(amp_signal)
    
    mi = mngs.dsp.modulation_index(phase[0, 0, :], amplitude[0, 0, :], n_bins=18)
...
def main(args):
    ...
    report = f"""
# MNGS DSP Module - Example Summary
...
"""
    ...
    mngs.io.save(report, "reports/dsp_examples_summary.md")
    print("\n" + "="*50)
