...
# File: ./examples/mngs/nn/neural_network_layers.py
# ----------------------------------------
import os

__FILE__ = "./examples/mngs/nn/neural_network_layers.py"
__DIR__ = os.path.dirname(__FILE__)
# ----------------------------------------

"""
Functionalities:
  - Demonstrates custom neural network layers in mngs.nn
...
Dependencies:
  - scripts: None
  - packages: torch, numpy, matplotlib, mngs
...
def demo_signal_processing_layers():
    ...
    filter_layer = mngs.nn.BandPassFilter(
    ...
    print("\n2. Hilbert Transform Layer")
    hilbert_layer = mngs.nn.Hilbert(seq_len=n_samples, dim=-1)
    ...
    print("\n3. PSD Layer")
    psd_layer = mngs.nn.PSD(sample_rate=sample_rate, prob=False, dim=-1)
...
def demo_augmentation_layers():
    ...
    print("\n4. Channel Gain Changer")
    channel_gain = mngs.nn.ChannelGainChanger(gain_range=(0.9, 1.1), p=0.5)
    channel_gain.train()
...
def demo_analysis_layers():
    ...
    phase_filter = mngs.nn.BandPassFilter(
    ...
    amp_filter = mngs.nn.BandPassFilter(
    ...
    hilbert = mngs.nn.Hilbert(seq_len=n_samples)
    ...
    mi_layer = mngs.nn.ModulationIndex(n_bins=18, fp16=False, amp_prob=False)

    mi_values = mi_layer(phase, amplitude)
    print(f"   MI values: {mi_values.item():.3f}")

    # 3. Spectrogram layer - Note: Not available in current mngs version
...
def demo_complete_model():
    ...
    class SignalClassifier(nn.Module):
        ...
        def __init__(self, n_channels=8, n_classes=2):
            ...
            self.filter = mngs.nn.BandPassFilter(
            ...
            # Feature extraction
            self.psd = mngs.nn.PSD(sample_rate=250.0, prob=False, dim=-1)
...
def visualize_layer_outputs():
    ...
    # 3. PSD features
    ax = axes[1, 0]
    psd_layer = mngs.nn.PSD(sample_rate=sample_rate)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Neural network layer examples with mngs.nn"
    )
