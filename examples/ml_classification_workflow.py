...
"""
Functionalities:
- Demonstrates MNGS for machine learning classification tasks
...
def plot_confusion_matrices(y_true, y_pred_dict, class_names, output_dir):
    ...
    fig, axes = mngs.plt.subplots(1, n_models, figsize=(6*n_models, 5))
    ...
    for idx, (model_name, y_pred) in enumerate(y_pred_dict.items()):
        ...
        # Plot using mngs
        mngs.plt.ax.plot.conf_mat(ax, y_true, y_pred, label_rotation=0)
...
def create_learning_curves(X, y, models, output_dir):
    ...
    fig, axes = mngs.plt.subplots(1, len(models), figsize=(6*len(models), 5))
