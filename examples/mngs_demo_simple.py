...
# File: ./examples/mngs_demo_simple.py

"""
Functionalities:
- Simple demonstration of core MNGS functionality
- Shows basic usage patterns without complex dependencies

Example usage:
$ python ./examples/mngs_demo_simple.py
...
- ./examples/mngs_demo_simple_out/: Directory containing outputs
"""
...
def main():
    ...
    filtered_signal = mngs.dsp.filt.bandpass(signal, fs, bands=[[5, 20]])
    ...
    # Compute PSD
    psd_values, freqs = mngs.dsp.psd(signal, fs)
    ...
    print("\n4. Saving outputs...")
    output_dir = os.path.join(os.getcwd(), "examples", "mngs_demo_simple_out")
    ...
    print("\n6. Data manipulation with mngs.pd:")
    # Force to DataFrame
    df_forced = mngs.pd.force_df(df)
    print(f"   Forced DataFrame shape: {df_forced.shape}")
    
    # Find indices
    indices_A = mngs.pd.find_indi(df, conditions={'group': 'A'})
