#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-04-27 16:39:26 (ywatanabe)"
# File: /ssh:sp:/home/ywatanabe/proj/mngs_repo/tests/mngs/plt/_subplots/test__to_sigma.py
# ----------------------------------------
import os
__FILE__ = (
    "./tests/mngs/plt/_subplots/test__to_sigma.py"
)
__DIR__ = os.path.dirname(__FILE__)
# ----------------------------------------

# Source code from: /home/ywatanabe/proj/mngs_dev/src/mngs/plt/_subplots/_to_sigma.py
# --------------------------------------------------------------------------------
# #!/usr/bin/env python3
# # -*- coding: utf-8 -*-
...
from mngs.plt._subplots._to_sigma import to_sigma


def test_to_sigma_scatter_combines_history():
    # prepare history with one scatter record
    x = np.array([1, 2, 3])
    y = np.array([4, 5, 6])
    record = ("sc1", "scatter", (x, y), {})
    history = {"r1": record}
    df = to_sigma(history)

    assert isinstance(df, pd.DataFrame)
    assert list(df.columns) == ["sc1_scatter_x", "sc1_scatter_y"]
    assert df["sc1_scatter_x"].tolist() == [1, 2, 3]
    assert df["sc1_scatter_y"].tolist() == [4, 5, 6]
