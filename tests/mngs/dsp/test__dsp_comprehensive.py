...
# File: /tests/mngs/dsp/test__dsp_comprehensive.py

"""
Comprehensive tests for mngs.dsp module core functions.
...
class TestDemoSig:
    ...
    def test_demo_sig_parameters(self):
        ...
        sig, time, fs_out = mngs.dsp.demo_sig(t_sec=t_sec, fs=fs)
...
class TestFiltering:
    ...
    def test_filt_bandpass(self, sample_signal):
        ...
        filtered = mngs.dsp.filt.bandpass(sig, sfreq, (30, 70))
    ...
    def test_filt_lowpass(self, sample_signal):
        ...
        filtered = mngs.dsp.filt.lowpass(sig, sfreq, 30)
    ...
    def test_filt_highpass(self, sample_signal):
        ...
        filtered = mngs.dsp.filt.highpass(sig, sfreq, 100)
...
class TestSpectralAnalysis:
    ...
    def test_signal(self):
        ...
        sig = (np.sin(2 * np.pi * 10 * t) +    # Alpha (10 Hz)
               np.sin(2 * np.pi * 20 * t) +    # Beta (20 Hz)
               0.5 * np.sin(2 * np.pi * 40 * t))  # Gamma (40 Hz)
    ...
    def test_psd_basic(self, test_signal):
        ...
        # Compute PSD
        psd, freqs = mngs.dsp.psd(sig, sfreq)
...
class TestTimeFrequencyAnalysis:
    ...
    def test_wavelet_basic(self):
        ...
        tfr = mngs.dsp.wavelet(sig, sfreq, freqs_wavelet)
...
class TestPAC:
    ...
    def test_pac_basic(self):
        ...
        pac_value = mngs.dsp.pac(sig, sfreq, pha_start_hz=8, pha_end_hz=12, amp_start_hz=45, amp_end_hz=55, pha_n_bands=10, amp_n_bands=10)
        ...
        pac_no_coupling = mngs.dsp.pac(sig_no_pac, sfreq, pha_start_hz=8, pha_end_hz=12, amp_start_hz=45, amp_end_hz=55, pha_n_bands=10, amp_n_bands=10)
...
class TestSignalProcessingUtils:
    ...
    def test_resample(self):
        ...
        # Resample to 500 Hz
        sfreq_new = 500
        sig_resampled = mngs.dsp.resample(sig_orig, sfreq_orig, sfreq_new)
    ...
    def test_crop(self):
        ...
        cropped = mngs.dsp.crop(sig, sfreq, 1.0, 3.0)
    ...
    def test_ensure_3d(self):
        ...
        sig_3d = mngs.dsp.ensure_3d(sig_2d)
        assert sig_3d.shape == (1, 5, 100)
        
        # Test 3D unchanged
        sig_3d_orig = np.random.randn(2, 5, 100)
        sig_3d = mngs.dsp.ensure_3d(sig_3d_orig)
...
class TestNormalization:
    ...
    def test_norm_minmax(self):
        ...
        normed = mngs.dsp.norm.minmax(sig)
...
class TestNoiseAddition:
    ...
    def test_add_white_noise(self):
        ...
        amp = 0.1
        noisy = mngs.dsp.add_noise.white(sig, amp=amp)
...
class TestRippleDetection:
    ...
    def test_detect_ripples(self):
        ...
        ripples = mngs.dsp.detect_ripples(sig, sfreq, low_hz=100, high_hz=200)
        ...
        for ripple in ripples:
            if (ripple['start'] <= ripple_start <= ripple['end'] or 
                ripple['start'] <= ripple_end <= ripple['end']):
                detected = True
                break
...
class TestModulationIndex:
    ...
    def test_modulation_index(self):
        ...
        mi = mngs.dsp.modulation_index(sig, sfreq, f_phase=(3, 7), f_amp=(35, 45))
        ...
        mi_unmod = mngs.dsp.modulation_index(sig_unmod, sfreq, f_phase=(3, 7), f_amp=(35, 45))
