...
# File: ./mngs_repo/tests/mngs/dsp/utils/test_pac.py
...
import mngs
from mngs.dsp.utils.pac import calc_pac_with_tensorpac, plot_PAC_mngs_vs_tensorpac
...
class TestCalcPacWithTensorpac:
    ...
    def test_calc_pac_with_tensorpac_mocked(self):
        ...
        with patch('mngs.dsp.utils.pac.tensorpac.Pac') as mock_pac_class:
...
class TestPlotPacMngsVsTensorpac:
    """Test plot_PAC_mngs_vs_tensorpac function."""
    
    @patch('mngs.plt.subplots')
    def test_plot_pac_basic(self, mock_subplots):
        ...
        # Test data
        pac_mngs = np.random.rand(50, 30)
        pac_tp = np.random.rand(50, 30)
        freqs_pha = np.linspace(1, 20, 50)
        freqs_amp = np.linspace(30, 150, 30)
        
        result = plot_PAC_mngs_vs_tensorpac(pac_mngs, pac_tp, freqs_pha, freqs_amp)
        ...
        mock_ax1.set_title.assert_called_with("mngs")
    ...
    def test_plot_pac_different_shapes(self, mock_subplots):
        ...
        for n_pha, n_amp in [(20, 15), (100, 50), (10, 10)]:
            pac_mngs = np.random.rand(n_pha, n_amp)
    ...
    def test_plot_pac_vmin_vmax_calculation(self, mock_subplots):
        ...
        pac_mngs = np.array([[0.1, 0.5], [0.3, 0.9]])
        pac_tp = np.array([[0.2, 0.4], [0.6, 0.8]])
        freqs_pha = np.array([1, 2])
        freqs_amp = np.array([30, 40])
        
        plot_PAC_mngs_vs_tensorpac(pac_mngs, pac_tp, freqs_pha, freqs_amp)
        
        # Expected vmin/vmax from the data and difference
        diff = pac_mngs - pac_tp
        expected_vmin = min(pac_mngs.min(), pac_tp.min(), diff.min())
        expected_vmax = max(pac_mngs.max(), pac_tp.max(), diff.max())
    ...
    def test_plot_pac_with_extreme_values(self, mock_subplots):
        ...
        pac_mngs = np.array([[0.0, 1000.0], [1e-10, 1e10]])
...
class TestPacIntegration:
    ...
    def test_pac_calculation_and_plotting_workflow(self, mock_subplots):
        ...
        with patch('mngs.dsp.utils.pac.tensorpac.Pac') as mock_pac_class:
            ...
            # Create mock mngs PAC result
            pac_mngs = np.random.rand(*pac_tp.shape)
    ...
    def test_pac_module_imports(self):
        ...
        assert callable(plot_PAC_mngs_vs_tensorpac)
...
# --------------------------------------------------------------------------------
# Start of Source Code from: /data/gpfs/projects/punim2354/ywatanabe/mngs_repo/src/mngs/dsp/utils/pac.py
# --------------------------------------------------------------------------------
# #! ./env/bin/python3
# # -*- coding: utf-8 -*-
...
# --------------------------------------------------------------------------------
# End of Source Code from: /data/gpfs/projects/punim2354/ywatanabe/mngs_repo/src/mngs/dsp/utils/pac.py
# --------------------------------------------------------------------------------
>>>>>>> origin/main
