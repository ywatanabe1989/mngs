...
# File: /tests/mngs/dsp/test__dsp_comprehensive_v2.py

"""
Comprehensive tests for mngs.dsp module - simplified version.
...
class TestDemoSig:
    ...
    def test_demo_sig_types(self):
        ...
        for sig_type in ["uniform", "gauss", "periodic", "chirp"]:
            sig, time, fs = mngs.dsp.demo_sig(sig_type=sig_type)
            assert isinstance(sig, np.ndarray)
            assert sig.ndim == 3
...
class TestBasicDSP:
    ...
    def sample_signal(self):
        ...
        sig, time, fs = mngs.dsp.demo_sig(t_sec=2.0, fs=1000, n_chs=1, batch_size=1)
        return sig, fs
    ...
    def test_crop(self):
        ...
        window_length = 20
        cropped = mngs.dsp.crop(sig, window_length=window_length)
    ...
    def test_resample(self, sample_signal):
        ...
        resampled = mngs.dsp.resample(sig, int(fs), int(new_fs))
...
class TestSpectralAnalysis:
    ...
    def test_psd(self):
        ...
        psd_result, freqs = mngs.dsp.psd(sig, fs=fs)
    ...
    def test_hilbert(self):
        ...
        phase, amplitude = mngs.dsp.hilbert(sig)
...
class TestFiltering:
    ...
    def test_filter_functions_exist(self):
        ...
        assert hasattr(mngs.dsp.filt, "bandpass")
        assert hasattr(mngs.dsp.filt, "lowpass")
        assert hasattr(mngs.dsp.filt, "highpass")
        assert hasattr(mngs.dsp.filt, "gauss")
...
class TestAddNoise:
    ...
    def test_add_white_noise(self):
        ...
        noisy = mngs.dsp.add_noise.white(sig, amp=0.1)
...
class TestPAC:
    ...
    def test_pac_basic(self):
        ...
        # Create signal
        sig, _, fs = mngs.dsp.demo_sig(sig_type="periodic", t_sec=5, fs=512)
...
class TestRippleDetection:
    ...
    def test_detect_ripples_basic(self):
        ...
        result = mngs.dsp.detect_ripples(sig, float(fs), low_hz=100, high_hz=200)
