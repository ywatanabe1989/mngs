#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Comprehensive tests for mngs.pd module.

This module contains comprehensive tests for all pandas-related utilities
in the mngs.pd module, covering DataFrame manipulation, column operations,
...
class TestDataFrameCreation:
    ...
    def test_force_df_with_dataframe(self):
        ...
        df = mngs.pd.force_df(original_df)
...
class TestColumnOperations:
    ...
    def test_merge_columns_basic(self):
        ...
        result = mngs.pd.merge_columns(df, ['first', 'last'])
        assert 'merged' in result.columns
        assert result['merged'].tolist() == ['first-John_last-Doe', 'first-Jane_last-Smith']
    ...
    def test_merge_columns_custom_separator(self):
        ...
        result = mngs.pd.merge_columns(df, ['year', 'month', 'day'], sep1='|', sep2='=')
        assert 'merged' in result.columns
        assert result['merged'].tolist() == ['year=2023|month=Jan|day=15', 'year=2024|month=Feb|day=20']
    ...
    def test_melt_cols(self):
        ...
        melted = mngs.pd.melt_cols(df, cols=['A_value', 'B_value', 'C_value'])
    ...
    def test_mv_to_first(self):
        ...
        result = mngs.pd.mv_to_first(df, 'c')
    ...
    def test_mv_to_last(self):
        ...
        result = mngs.pd.mv_to_last(df, 'b')
...
class TestSearchAndFilter:
    ...
    def test_find_indi_exact_match(self):
        ...
        indices = mngs.pd.find_indi(df, {'name': 'Bob'})
        assert indices == [1]
        
        # Find by multiple criteria
        indices = mngs.pd.find_indi(df, {'age': 30})
        assert indices == [1, 3]
    ...
    def test_find_pval_columns(self):
        ...
        pval_cols = mngs.pd.find_pval(df)
...
class TestDataManipulation:
    ...
    def test_slice_dataframe(self):
        ...
        # Slice rows
        sliced = mngs.pd.slice(df, slice(2, 5))
        assert len(sliced) == 3
        assert sliced.index.tolist() == [2, 3, 4]
        
        # Slice with step
        sliced = mngs.pd.slice(df, slice(0, 10, 2))
        assert len(sliced) == 5
    ...
    def test_sort_dataframe(self):
        ...
        sorted_df = mngs.pd.sort(df, ['rank', 'score'])
...
class TestUtilities:
    ...
    def test_ignore_setting_with_copy_warning(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            with mngs.pd.ignore_SettingWithCopyWarning():
                df_view['a'] = 100
            
            # Check no SettingWithCopyWarning was raised
            setting_warnings = [warning for warning in w 
...
class TestEdgeCases:
    ...
    def test_empty_dataframe(self):
        ...
        result = mngs.pd.force_df(empty_df)
        assert isinstance(result, pd.DataFrame)
        assert len(result) == 0
        
        # find_indi should return empty list
        indices = mngs.pd.find_indi(empty_df, {})
        assert indices == []
    ...
    def test_single_row_dataframe(self):
        ...
        sorted_df = mngs.pd.sort(single_row, 'a')
        assert len(sorted_df) == 1
        
        rounded = mngs.pd.round(single_row, 0)
    ...
    def test_mixed_types(self):
        ...
        result = mngs.pd.force_df(df)
        assert len(result) == 5
        
        # to_numeric should convert what it can
        numeric = mngs.pd.to_numeric(result)
...
class TestIntegration:
    ...
    def test_data_pipeline(self):
        ...
        df = mngs.pd.force_df(raw_data)
        
        # Merge name columns
        df = mngs.pd.merge_columns(df, ['first_name', 'last_name'], sep=' ')
        
        # Convert scores to numeric (keep text columns as is)
        df = mngs.pd.to_numeric(df, errors='ignore')
        ...
        # Round averages
        df = mngs.pd.round(df, 1)
        
        # Sort by average score
        df = mngs.pd.sort(df, 'avg_score', ascending=False)
        
        # Verify pipeline results
        assert 'first_name_last_name' in df.columns
        assert df['first_name_last_name'].iloc[0] in ['John Doe', 'Jane Smith', 'Bob Jones']
