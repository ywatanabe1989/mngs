...
# File: /data/gpfs/projects/punim2354/ywatanabe/.claude-worktree/mngs_repo/tests/mngs/etc/test_etc_performance.py
# ----------------------------------------
...
__FILE__ = "./tests/mngs/etc/test_etc_performance.py"
...
class TestEtcPerformance:
    ...
    def test_wait_key_response_time(self, benchmark_results):
        ...
        for num_keys in [10, 100, 1000]:
            ...
            with patch('mngs.etc.wait_key.readchar.readchar', side_effect=keys):
    ...
    def test_wait_key_cpu_usage(self):
        ...
        with patch('mngs.etc.wait_key.readchar.readchar', side_effect=key_generator()):
            with patch('builtins.print'):
                wait_key(mock_process)
    ...
    def test_count_performance_scaling(self, benchmark_results):
        ...
        for iterations in [100, 1000, 10000]:
            ...
            with patch('mngs.etc.wait_key.time.sleep'):  # Remove sleep for perf test
    ...
    def test_concurrent_wait_key_performance(self):
        ...
        def run_wait_key(thread_id):
            ...
            with patch('mngs.etc.wait_key.readchar.readchar', side_effect=['a', 'b', 'c', 'q']):
                with patch('builtins.print'):
                    wait_key(mock_process)
    ...
    def test_input_latency(self):
        ...
        for seq_length in [10, 50, 100]:
            ...
            with patch('mngs.etc.wait_key.readchar.readchar', side_effect=reader.readchar):
                with patch('builtins.print'):
                    wait_key(mock_process)
    ...
    def test_count_with_real_sleep_performance(self):
        ...
        with patch('mngs.etc.wait_key.time.sleep', lambda x: time.sleep(0.001)):  # 1ms sleep
            with patch('builtins.print', side_effect=mock_print):
                try:
                    count()
                except KeyboardInterrupt:
    ...
    def test_termination_speed(self):
        ...
        for _ in range(10):
            ...
            with patch('mngs.etc.wait_key.readchar.readchar', side_effect=mock_readchar):
                with patch('builtins.print'):
                    wait_key(mock_process)
    ...
    def test_scalability_limits(self):
        ...
        with patch('mngs.etc.wait_key.readchar.readchar', side_effect=huge_key_sequence):
            with patch('builtins.print'):
                wait_key(mock_process)
