...
# File: /data/gpfs/projects/punim2354/ywatanabe/.claude-worktree/mngs_repo/tests/mngs/etc/test_wait_key_enhanced.py
# ----------------------------------------
...
__FILE__ = "./tests/mngs/etc/test_wait_key_enhanced.py"
...
class TestWaitKeyEnhanced:
    ...
    def test_wait_key_with_random_key_sequences(self, key_sequence):
        ...
        with patch('mngs.etc.wait_key.readchar.readchar', side_effect=full_sequence):
    ...
    def test_wait_key_with_process_lifecycle(self):
        ...
        with patch('mngs.etc.wait_key.readchar.readchar', side_effect=['a', 'b', 'q']):
            with patch('builtins.print'):
                wait_key(mock_process)
    ...
    def test_count_with_interruption_timing(self):
        ...
        with patch('mngs.etc.wait_key.time.sleep', lambda x: time.sleep(0.01)):  # Short sleep
            with patch('builtins.print', side_effect=mock_print):
                try:
                    count()
                except KeyboardInterrupt:
    ...
    def test_wait_key_unicode_handling(self):
        ...
        with patch('mngs.etc.wait_key.readchar.readchar', side_effect=unicode_chars):
            with patch('builtins.print') as mock_print:
                wait_key(mock_process)
