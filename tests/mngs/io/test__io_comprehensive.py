...
"""
Comprehensive tests for mngs.io module core functions.
...
class TestLoadSaveRoundtrip:
    ...
    def test_pickle_roundtrip(self, temp_dir, sample_data):
        ...
        for name, data in sample_data.items():
            ...
            # Act
            mngs.io.save(data, file_path)
            loaded = mngs.io.load(file_path)
    ...
    def test_numpy_roundtrip(self, temp_dir):
        ...
        # Test .npy
        mngs.io.save(data, npy_path)
        loaded_npy = mngs.io.load(npy_path)
        np.testing.assert_array_equal(loaded_npy, data)
        
        # Test .npz with multiple arrays
        data_dict = {'arr1': data, 'arr2': data * 2}
        mngs.io.save(data_dict, npz_path)
        loaded_npz = mngs.io.load(npz_path)
    ...
    def test_json_roundtrip(self, temp_dir):
        ...
        # Act
        mngs.io.save(data, json_path)
        loaded = mngs.io.load(json_path)
    ...
    def test_yaml_roundtrip(self, temp_dir):
        ...
        # Act
        mngs.io.save(data, yaml_path)
        loaded = mngs.io.load(yaml_path)
    ...
    def test_csv_roundtrip(self, temp_dir):
        ...
        # Act
        mngs.io.save(df, csv_path)
        loaded = mngs.io.load(csv_path)
    ...
    def test_text_roundtrip(self, temp_dir):
        ...
        # Act
        mngs.io.save(text, txt_path)
        loaded = mngs.io.load(txt_path)
    ...
    def test_excel_roundtrip(self, temp_dir):
        ...
        # Act
        mngs.io.save(df, excel_path)
        loaded = mngs.io.load(excel_path)
    ...
    def test_hdf5_roundtrip(self, temp_dir):
        ...
        # Act
        mngs.io.save(data, h5_path)
        loaded = mngs.io.load(h5_path)
...
class TestLoadFunctionality:
    ...
    def test_load_nonexistent_file(self):
        ...
        with pytest.raises((FileNotFoundError, IOError)):
            mngs.io.load("nonexistent_file.pkl")
    ...
    def test_load_with_glob_pattern(self, temp_dir):
        ...
        for i in range(3):
            ...
            mngs.io.save(data, os.path.join(temp_dir, f"data_{i}.json"))
        
        # Load with pattern
        pattern = os.path.join(temp_dir, "data_*.json")
        loaded = mngs.io.load(pattern)
    ...
    def test_load_compressed_files(self, temp_dir):
        ...
        loaded = mngs.io.load(pkl_gz_path)
        assert loaded == data
...
class TestSaveFunctionality:
    ...
    def test_save_creates_directories(self, temp_dir):
        ...
        # Act
        mngs.io.save(data, nested_path)
        
        # Assert
        assert os.path.exists(nested_path)
        loaded = mngs.io.load(nested_path)
        assert loaded == data
    ...
    def test_save_overwrites_existing(self, temp_dir):
        ...
        # Act
        mngs.io.save(data1, file_path)
        mngs.io.save(data2, file_path)  # Overwrite
    ...
    def test_save_complex_pandas_dataframe(self, temp_dir):
        ...
        for ext in ['.pkl', '.csv', '.xlsx']:
            ...
            if ext in ['.csv', '.xlsx']:
                ...
                if ext == '.xlsx':
                    assert loaded['category_col'].tolist() == df['category_col'].tolist()
            else:
                pd.testing.assert_frame_equal(loaded, df)
...
class TestSpecialCases:
    ...
    def test_large_data_handling(self, temp_dir):
        ...
        # Save and load
        mngs.io.save(large_array, file_path)
        loaded = mngs.io.load(file_path)
    ...
    def test_special_characters_in_path(self, temp_dir):
        ...
        # Save and load
        mngs.io.save(data, file_path)
        # Note: mngs.io.save cleans paths by replacing spaces with underscores
        # So we need to load from the cleaned path
        cleaned_path = file_path.replace(" ", "_")
        loaded = mngs.io.load(cleaned_path)
    ...
    def test_format_inference(self, temp_dir):
        ...
        for ext, module in extensions.items():
            ...
            # Verify mngs.io.load gives same result
            mngs_load = mngs.io.load(file_path)
            assert mngs_load == native_load


class TestGlobFunctionality:
    """Test mngs.io.glob functionality."""
    ...
    def temp_dir_with_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i in range(3):
                ...
                mngs.io.save({'exp': i, 'type': 'config'}, 
                           os.path.join(subdir, 'config.json'))
                mngs.io.save(np.random.randn(10, 5), 
                           os.path.join(subdir, 'data.npy'))
                mngs.io.save(f"Results for experiment {i}", 
                           os.path.join(subdir, 'results.txt'))
    ...
    def test_glob_pattern_matching(self, temp_dir_with_files):
        ...
        # Find all JSON files
        json_files = mngs.io.glob(os.path.join(temp_dir_with_files, '**/config.json'))
        assert len(json_files) == 3
        assert all(f.endswith('config.json') for f in json_files)
        
        # Find all numpy files
        npy_files = mngs.io.glob(os.path.join(temp_dir_with_files, '**/*.npy'))
        assert len(npy_files) == 3
        
        # Find files in specific subdirectory
        exp0_files = mngs.io.glob(os.path.join(temp_dir_with_files, 'exp_0/*'))
    ...
    def test_glob_with_load(self, temp_dir_with_files):
        ...
        config_files = mngs.io.glob(config_pattern)
        
        configs = [mngs.io.load(f) for f in config_files]
