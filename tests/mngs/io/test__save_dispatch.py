...
# File: /data/gpfs/projects/punim2354/ywatanabe/mngs_repo/tests/mngs/io/test__save_dispatch.py
# ----------------------------------------
import os

__FILE__ = "./tests/mngs/io/test__save_dispatch.py"
...
import mngs
from mngs.io._save import _FILE_HANDLERS


class TestSaveDispatchDictionary:
    """Test cases for the dispatch dictionary optimization in mngs.io.save."""
    ...
    def test_dispatch_dictionary_exists(self):
        ...
        assert hasattr(mngs.io._save, '_FILE_HANDLERS')
    ...
    def test_dispatch_performance(self, temp_dir):
        ...
        for ext in extensions:
            ...
            if ext == '.npy':
                mngs.io.save(np.array([1, 2, 3]), save_path, verbose=False)
            elif ext == '.txt':
                mngs.io.save("test text", save_path, verbose=False)
            else:
    ...
    def test_excel_handler(self, temp_dir):
        ...
        for ext in ['.xlsx', '.xls']:
            save_path = os.path.join(temp_dir, f"test{ext}")
            mngs.io.save(df, save_path, verbose=False)
    ...
    def test_numpy_handlers(self, temp_dir):
        ...
        mngs.io.save(arr, npy_path, verbose=False)
        ...
        mngs.io.save(npz_data, npz_path, verbose=False)
    ...
    def test_text_format_handlers(self, temp_dir):
        ...
        mngs.io.save(json_data, json_path, verbose=False)
        assert os.path.exists(json_path)
        
        # YAML
        yaml_path = os.path.join(temp_dir, "test.yaml")
        mngs.io.save(json_data, yaml_path, verbose=False)
        ...
        for ext in ['.txt', '.md', '.py']:
            ...
            mngs.io.save(text_data, text_path, verbose=False)
    ...
    def test_torch_handlers(self, temp_dir):
        ...
        for ext in ['.pth', '.pt']:
            save_path = os.path.join(temp_dir, f"test{ext}")
            mngs.io.save(tensor, save_path, verbose=False)
            assert os.path.exists(save_path)
    ...
    def test_image_handler_special_params(self, temp_dir):
        ...
        with patch('mngs.io._save._save_image') as mock_save_image:
            png_path = os.path.join(temp_dir, "test.png")
            mngs.io.save(fig, png_path, verbose=False, no_csv=True)
    ...
    def test_unsupported_format_error(self, temp_dir):
        ...
        with pytest.raises(ValueError, match="Unsupported file format"):
            mngs.io.save(data, unsupported_path, verbose=False)
    ...
    def test_special_extension_handling(self, temp_dir):
        ...
        mngs.io.save(data, pkl_gz_path, verbose=False)
    ...
    def test_csv_special_handling(self, temp_dir):
        ...
        mngs.io.save(df, csv_path, verbose=False)
