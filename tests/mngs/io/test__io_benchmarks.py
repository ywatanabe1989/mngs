...
# File: /tests/mngs/io/test__io_benchmarks.py
# ----------------------------------------
"""
Performance benchmark tests for mngs.io module.
...
class TestIOPerformance:
    ...
    def test_memory_usage_large_files(self, temp_dir):
        ...
        # Save data
        mngs.io.save(large_data, path, verbose=False)
    ...
    def test_compression_effectiveness(self, temp_dir):
        ...
        for data, desc, expected_ratio in test_cases:
            ...
            start = time.time()
            mngs.io.save(data, path_raw, verbose=False)
            ...
            start = time.time()
            mngs.io.save({'data': data}, path_compressed, compress=True, verbose=False)
    ...
    def test_performance_regression_suite(self, temp_dir, benchmark_datasets):
        ...
        for (operation, size), expected_time in baseline_performance.items():
            ...
            if operation.startswith('save'):
                start = time.time()
                mngs.io.save(test_data, path, verbose=False)
...
# Run with: pytest -m benchmark tests/mngs/io/test__io_benchmarks.py
# For detailed output: pytest -v -s -m benchmark tests/mngs/io/test__io_benchmarks.py
