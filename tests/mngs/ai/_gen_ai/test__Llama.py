...
# File: ./tests/mngs/ai/_gen_ai/test__Llama.py

"""Tests for mngs.ai._gen_ai._Llama module."""
...
class TestLlama:
    ...
    def mock_llama_module(self):
        ...
        with patch('mngs.ai._gen_ai._Llama.Dialog') as mock_dialog:
            with patch('mngs.ai._gen_ai._Llama._Llama') as mock_llama_class:
                yield mock_dialog, mock_llama_class

    def test_init_with_default_paths(self, mock_env_setup):
        """Test initialization with default paths."""
        from mngs.ai._gen_ai._Llama import Llama
        
        with patch('mngs.ai._gen_ai._Llama.MODELS', MagicMock()):
    ...
    def test_environment_setup(self, mock_env_setup):
        ...
        with patch('mngs.ai._gen_ai._Llama.MODELS', MagicMock()):
            with patch.object(Llama, '_init_client', return_value=Mock()):
                with patch('mngs.ai._gen_ai._Llama.print_envs') as mock_print:
                    llama_ai = Llama(model="Meta-Llama-3-8B")
