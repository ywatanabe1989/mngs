...
class TestGenAIModuleInit:
    def test_models_import(self):
        from mngs.ai._gen_ai import MODELS
    ...
    def test_models_dataframe_structure(self):
        ...
        for col in required_columns:
            assert col in MODELS.columns, f"Column '{col}' missing from MODELS DataFrame"

    def test_models_contains_expected_providers(self):
        from mngs.ai._gen_ai import MODELS
        
        providers = MODELS['provider'].unique()
        expected_providers = ['OpenAI', 'Anthropic', 'Google', 'Perplexity', 'Llama', 'DeepSeek', 'Groq']
        
        for provider in expected_providers:
            assert provider in providers, f"Provider '{provider}' not found in MODELS"
    ...
    def test_models_cost_structure(self):
        ...
        for _, model in MODELS.iterrows():
            ...
            if not pd.isna(model['input_cost']) and model['input_cost'] is not None:
                assert model['input_cost'] >= 0, f"Negative input cost for {model['name']}"
            if not pd.isna(model['output_cost']) and model['output_cost'] is not None:
                assert model['output_cost'] >= 0, f"Negative output cost for {model['name']}"
    ...
    def test_models_api_key_env_format(self):
        ...
        for _, model in MODELS.iterrows():
            ...
            assert isinstance(api_key_env, str), f"API key env should be string for {model['name']}"
            assert api_key_env.endswith('_API_KEY'), f"API key env should end with '_API_KEY' for {model['name']}"
    ...
    def test_models_name_uniqueness(self):
        ...
        assert len(model_names) == len(unique_names), "Duplicate model names found in MODELS"
...
class TestGenAIModuleImportStructure:
    ...
    def test_import_from_params(self):
        ...
        from mngs.ai._gen_ai._PARAMS import MODELS as PARAMS_MODELS
        from mngs.ai._gen_ai import MODELS
        
        # Should be the same object/data
        pd.testing.assert_frame_equal(MODELS, PARAMS_MODELS)

    def test_module_attributes(self):
        import mngs.ai._gen_ai as gen_ai_module
...
class TestGenAIModelDataValidation:
    def test_models_data_integrity(self):
        ...
        for _, model in MODELS.iterrows():
            ...
            assert model['provider'].strip() != '', f"Empty provider found for {model['name']}"
            assert model['api_key_env'].strip() != '', f"Empty api_key_env found for {model['name']}"
    ...
    def test_models_provider_consistency(self):
        ...
        for provider_name, group in providers:
            ...
            assert len(api_keys) == 1, f"Provider {provider_name} has inconsistent API key env vars: {api_keys}"
    ...
    def test_models_cost_ranges(self):
        ...
        for _, model in MODELS.iterrows():
            if not pd.isna(model['input_cost']) and model['input_cost'] is not None:
                assert 0 <= model['input_cost'] <= 100, f"Unreasonable input cost for {model['name']}: {model['input_cost']}"
            if not pd.isna(model['output_cost']) and model['output_cost'] is not None:
                assert 0 <= model['output_cost'] <= 100, f"Unreasonable output cost for {model['name']}: {model['output_cost']}"
    ...
    def test_models_name_format(self):
        ...
        for _, model in MODELS.iterrows():
            ...
            assert name.islower() or any(char.isdigit() for char in name), f"Unusual name format: {name}"
...
class TestGenAIModelSpecificValidation:
    def test_openai_model_specifics(self):
        ...
        for _, model in openai_models.iterrows():
            ...
            if not pd.isna(model['input_cost']):
                assert model['input_cost'] >= 0, f"OpenAI model {model['name']} has negative input cost"
            if not pd.isna(model['output_cost']):
                assert model['output_cost'] >= 0, f"OpenAI model {model['name']} has negative output cost"
    ...
    def test_google_model_specifics(self):
        ...
        for _, model in google_models.iterrows():
            assert model['api_key_env'] == 'GOOGLE_API_KEY', f"Google model {model['name']} has wrong API key env"
    ...
    def test_model_count_reasonableness(self):
        ...
        assert 10 <= total_models <= 200, f"Unusual number of models: {total_models}"
...
# --------------------------------------------------------------------------------
# Start of Source Code from: /data/gpfs/projects/punim2354/ywatanabe/mngs_repo/src/mngs/ai/_gen_ai/__init__.py
# --------------------------------------------------------------------------------
# #!./env/bin/python3
# # -*- coding: utf-8 -*-
...
# --------------------------------------------------------------------------------
# End of Source Code from: /data/gpfs/projects/punim2354/ywatanabe/mngs_repo/src/mngs/ai/_gen_ai/__init__.py
# --------------------------------------------------------------------------------
>>>>>>> origin/main
