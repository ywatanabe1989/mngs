...
# File: ./mngs_repo/tests/mngs/ai/genai/test_base_genai.py

"""Tests for mngs.ai.genai.base_genai module using file-based structure approach.
...
class TestBaseGenAIModule:
    ...
    def test_base_genai_module_has_mngs_integration(self):
        """Test that BaseGenAI module integrates with mngs framework."""
        module_path = os.path.join(
            os.path.dirname(__file__), '..', '..', '..', '..', 'src', 
            'mngs', 'ai', 'genai', 'base_genai.py'
        )
        ...
        assert 'import mngs' in content, "BaseGenAI should import mngs"
        assert 'mngs.gen.start' in content, "BaseGenAI should use mngs.gen.start"
        assert 'mngs.gen.close' in content, "BaseGenAI should use mngs.gen.close"
