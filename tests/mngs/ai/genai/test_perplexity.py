...
# File: ./mngs_repo/tests/mngs/ai/genai/test_perplexity.py

"""Tests for mngs.ai.genai.perplexity module using file-based structure approach.
...
class TestPerplexityModule:
    ...
    def test_perplexity_module_has_mngs_integration(self):
        """Test that Perplexity module integrates with mngs framework."""
        module_path = os.path.join(
            os.path.dirname(__file__), '..', '..', '..', '..', 'src', 
            'mngs', 'ai', 'genai', 'perplexity.py'
        )
        ...
        assert 'import mngs' in content, "Perplexity should import mngs"
        assert 'mngs.gen.start' in content, "Perplexity should use mngs.gen.start"
        assert 'mngs.gen.close' in content, "Perplexity should use mngs.gen.close"
