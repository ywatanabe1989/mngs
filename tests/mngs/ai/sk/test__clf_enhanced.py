...
# File: /tests/mngs/ai/sk/test__clf_enhanced.py
# ----------------------------------------
"""
Enhanced tests for mngs.ai.sk._clf module implementing advanced testing patterns.
...
try:
    ...
    pytest.skip("mngs.ai.sk module not available", allow_module_level=True)
...
class TestPipelineWithMocks:
    ...
    @patch('mngs.ai.sk._clf.LogisticRegression')
    ...
    def test_gb_pipeline_mocked_components(self, mock_components):
        ...
        with patch('mngs.ai.sk._clf.Tabularizer', return_value=mock_components['tabularizer']):
            with patch('mngs.ai.sk._clf.GradientBoostingClassifier', 
                      return_value=mock_components['gradient_boosting']):
                
                # Need to recreate the pipeline with mocked components
                from mngs.ai.sk._clf import make_pipeline
