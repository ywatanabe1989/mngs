...
# File: ./mngs_repo/tests/mngs/web/test___init__.py
...
class TestWebModuleInit:
    ...
    def test_cleanup_of_temporary_variables(self):
        ...
        if 'mngs.web' in sys.modules:
            del sys.modules['mngs.web']
        
        import mngs.web
        
        # Check that temporary variables don't exist in the module namespace
        assert not hasattr(mngs.web, 'filename')
        assert not hasattr(mngs.web, 'module_name')
        assert not hasattr(mngs.web, 'module')
        assert not hasattr(mngs.web, 'name')
        assert not hasattr(mngs.web, 'obj')
    ...
    def test_only_public_functions_exported(self):
        ...
        # Get all attributes
        all_attrs = dir(mngs.web)
        
        # Filter for actual web module functions (not built-ins)
        web_functions = [
            attr for attr in all_attrs 
            if not attr.startswith('__') and callable(getattr(mngs.web, attr))
        ]
    ...
    def test_search_pubmed_submodule_functions(self):
        ...
        for func_name in expected_functions:
            assert hasattr(mngs.web, func_name), f"Function {func_name} should be available"
            assert callable(getattr(mngs.web, func_name)), f"{func_name} should be callable"
    ...
    def test_no_submodule_conflicts(self):
        ...
        attrs = {name: getattr(mngs.web, name) for name in dir(mngs.web) if not name.startswith('_')}
    ...
    def test_module_reimport_consistency(self):
        ...
        # First import
        import mngs.web
        functions_first = set(name for name in dir(mngs.web) if callable(getattr(mngs.web, name)) and not name.startswith('_'))
        
        # Force reimport
        importlib.reload(mngs.web)
        functions_second = set(name for name in dir(mngs.web) if callable(getattr(mngs.web, name)) and not name.startswith('_'))
    ...
    def test_module_file_path(self):
        ...
        assert hasattr(mngs.web, '__file__')
        assert mngs.web.__file__.endswith('__init__.py')
        assert 'web' in mngs.web.__file__
    ...
    def test_async_functions_available(self):
        ...
        for func_name in async_functions:
            if hasattr(mngs.web, func_name):
                func = getattr(mngs.web, func_name)
...
# --------------------------------------------------------------------------------
# Start of Source Code from: /data/gpfs/projects/punim2354/ywatanabe/mngs_repo/src/mngs/web/__init__.py
# --------------------------------------------------------------------------------
# #!/usr/bin/env python3
# # -*- coding: utf-8 -*-
...
# # EOF
# --------------------------------------------------------------------------------
# End of Source Code from: /data/gpfs/projects/punim2354/ywatanabe/mngs_repo/src/mngs/web/__init__.py
# --------------------------------------------------------------------------------
>>>>>>> origin/main
