...
#             Name of new column
#         column_type : str
#             SQL type of new column (e.g., 'INTEGER', 'TEXT')
...
#         >>> db.add_column('posts', 'status', 'TEXT', default_value="'draft'")
# 
#         Raises
#         ------
#         ValueError
...
#                         raise ValueError(f"Column {column} not found in {src_table}")
# 
#                     current_schema = self.get_table_schema(tgt_table)
#                     if column in current_schema:
#                         raise ValueError(f"Column {column} already exists in {tgt_table}")
# 
...
#                     self.execute(f"INSERT INTO {temp_table} ({old_cols}) SELECT {old_cols} FROM {tgt_table}")
# 
#                     # Drop old table and rename new one
#                     self.execute(f"DROP TABLE {tgt_table}")
#                     self.execute(f"ALTER TABLE {temp_table} RENAME TO {tgt_table}")
# 
#                     # Add foreign key constraint
#                     query = (f"ALTER TABLE {tgt_table} ADD FOREIGN KEY ({column}) "
#                             f"REFERENCES {src_table}({column})")
#                     self.execute(query)
# 
#                     # Update with default value if provided
#                     if default_value is not None:
#                         self.execute(f"UPDATE {tgt_table} SET {column} = ?", (default_value,))
# 
#         except sqlite3.Error as err:
#             raise ValueError(f"Failed to add foreign key column: {err}")
# 
#         # try:
#         #     # Verify tables exist and get schemas
#         #     src_schema = self.get_table_schema(src_table)
#         #     if src_column not in src_schema:
#         #         raise ValueError(f"Column {src_column} not found in {src_table}")
# 
#         #     current_schema = self.get_table_schema(tgt_table)
#         #     if tgt_column in current_schema:
#         #         raise ValueError(f"Column {tgt_column} already exists in {tgt_table}")
# 
#         #     column_type = src_schema[src_column]
# 
#         #     # Create new table schema with foreign key first
#         #     new_columns = {tgt_column: column_type}
#         #     new_columns.update(current_schema)
# 
#         #     # Enable foreign keys
#         #     self.execute("PRAGMA foreign_keys=ON")
...
# 
#         #     # Add foreign key constraint
#         #     query = (f"ALTER TABLE {tgt_table} ADD FOREIGN KEY ({tgt_column}) "
#         #             f"REFERENCES {src_table}({src_column})")
#         #     self.execute(query)
# 
#         #     # Update new column with default if provided
#         #     if default_value is not None:
#         #         self.execute(f"UPDATE {tgt_table} SET {tgt_column} = {default_value}")
# 
#         # except sqlite3.Error as err:
#         #     raise ValueError(f"Failed to add foreign key column: {err}")
# 
...
#     #     valid_columns = [col for col in rows[0].keys()]# if col in table_columns]
# 
#     #     if inherit_foreign:
...
#     #     query = f"INSERT OR REPLACE INTO {table_name} ({','.join(valid_columns)}) VALUES ({placeholders})"
# 
...
#     #     columns = valid_columns if 'id' in valid_columns else ['id'] + valid_columns
#     #     placeholders = ",".join(["?" for _ in columns])
#     #     query = f"REPLACE INTO {table_name} ({','.join(columns)}) VALUES ({placeholders})"
# 
#     #     for idx in range(0, len(rows), batch_size):
#     #         batch = rows[idx : idx + batch_size]
#     #         values = [
...
#                     if (
#                         dtype_col in table_sample.columns
#                         and shape_col in table_sample.columns
#                     ):
#                         table_sample[column] = table_sample.apply(
#                             lambda row: f"<BLOB: shape={row[shape_col]}, dtype={row[dtype_col]}>",
#                             axis=1,
#                         )
#                     else:
#                         table_sample[column] = "<BLOB>"
# 
...
from src.mngs.db/_BaseSQLiteDB.py import *
