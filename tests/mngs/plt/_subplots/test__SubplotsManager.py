#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-04-27 16:39:24 (ywatanabe)"
# File: /ssh:sp:/home/ywatanabe/proj/mngs_repo/tests/mngs/plt/_subplots/test__SubplotsManager.py
# ----------------------------------------
import os
__FILE__ = (
...
# Source code from: /home/ywatanabe/proj/mngs_dev/src/mngs/plt/_subplots/_SubplotsManager.py
# --------------------------------------------------------------------------------
# #!./env/bin/python3
# # -*- coding: utf-8 -*-
...
from mngs.plt._subplots._SubplotsManager import subplots


def test_subplots_scatter_tracking_and_to_sigma():
    fig, ax = subplots()
    x = [10, 20]
    y = [30, 40]
    ax.scatter(x, y, id="s1")
    df = ax.to_sigma()

    assert isinstance(df, pd.DataFrame)
    assert list(df.columns) == ["s1_scatter_x", "s1_scatter_y"]
...
def test_full_workflow_to_sigma():
    # 1. line
    fig_line, ax_line = subplots()
    ax_line.plot([1, 2, 3], [4, 5, 6], id="plot1")
    ax_line.plot([4, 5, 6], [1, 2, 3], id="plot2")
    df_line = ax_line.to_sigma()
    assert df_line.columns.tolist() == [
        "plot1_plot_x",
        "plot1_plot_y",
        "plot2_plot_x",
        "plot2_plot_y",
    ...
    # 2. no-tracking
    _, ax_no_track = subplots(track=False)
    ax_no_track.plot([1, 2, 3], [4, 5, 6], id="plot3")
    assert ax_no_track.to_sigma().empty

    # 3. scatter
    _, ax_scatter = subplots()
    ax_scatter.scatter([1, 2, 3], [4, 5, 6], id="scatter1")
    df_scatter = ax_scatter.to_sigma()
    assert df_scatter.columns.tolist() == [
        "scatter1_scatter_x",
        "scatter1_scatter_y",
    ]
    ...
    # 4. box + bar
    _, ax_mix = subplots()
    ax_mix.boxplot([1, 2, 3], id="boxplot1")
    ax_mix.bar(["A", "B", "C"], [4, 5, 6], id="bar1")
    df_mix = ax_mix.to_sigma()
    assert set(["boxplot1_boxplot_x", "bar1_bar_x", "bar1_bar_y"]).issubset(
        df_mix.columns
    )
