...
# File: /home/ywatanabe/proj/mngs_repo/tests/mngs/plt/ax/_plot/test__plot_heatmap_enhanced.py
# ----------------------------------------
import os
import sys
...
from mngs.plt.ax._plot._plot_heatmap import plot_heatmap
...
def mock_save():
    ...
    with patch('mngs.io.save') as mock:
        yield mock
...
class TestMngsIntegration:
    ...
    def test_with_mngs_wrapper(self, sample_data):
        """Test with mngs plotting wrapper if available."""
        try:
            ...
            # Should work seamlessly with mngs wrapper
            assert hasattr(ax, '__class__')
            
            plt.close(fig)
        except ImportError:
            pytest.skip("mngs.plt.subplots not available")
