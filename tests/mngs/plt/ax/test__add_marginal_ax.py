#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-04-27 13:29:20 (ywatanabe)"
...
# ----------------------------------------
# Source code from: /home/ywatanabe/proj/_mngs_repo/src/mngs/plt/ax/_add_marginal_ax.py
# --------------------------------------------------------------------------------
# #!./env/bin/python3
# # -*- coding: utf-8 -*-
...
def test_add_marginal_ax_basic():
    fig, ax = plt.subplots()
    # create top marginal
    ax_top = add_marginal_ax(ax, "top", size=0.3, pad=0.1)
    assert ax_top is not ax
    assert ax_top.get_figure() is fig


def test_add_marginal_ax_left_right():
    fig, ax = plt.subplots()
    for place in ("left", "right"):
        ax_side = add_marginal_ax(ax, place)
        assert ax_side.get_figure() is fig
