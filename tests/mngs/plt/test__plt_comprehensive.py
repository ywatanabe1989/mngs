...
# File: /tests/mngs/plt/test__plt_comprehensive.py

"""
Comprehensive tests for mngs.plt module.
...
class TestSubplotsWrapper:
    ...
    def test_subplots_with_data_tracking(self, temp_dir):
        ...
        if hasattr(ax, '_mngs_data'):
            assert len(ax._mngs_data) > 0
...
class TestPlottingFunctions:
    ...
    def test_plot_with_style(self):
        """Test plotting with mngs style enhancements."""
        # Create figure
        fig, ax = mngs.plt.subplots()
    ...
    def test_color_palette(self):
        """Test mngs color palette functionality."""
        # Get colors
        if hasattr(mngs.plt, 'get_colors'):
            colors = mngs.plt.get_colors()
...
class TestSpecializedPlots:
    ...
    def test_heatmap_if_available(self):
        ...
        if hasattr(mngs.plt, 'heatmap') or hasattr(mngs.plt.ax, 'heatmap'):
            # Create sample data
            data = np.random.randn(10, 10)
            
            # Create heatmap
            fig, ax = mngs.plt.subplots()
            if hasattr(mngs.plt, 'heatmap'):
                mngs.plt.heatmap(data, ax=ax)
            elif hasattr(ax, 'heatmap'):
                ax.heatmap(data)
    
    def test_confusion_matrix_if_available(self):
        """Test confusion matrix plotting if available."""
        if hasattr(mngs.plt, 'confusion_matrix') or hasattr(mngs.plt, 'plot_confusion_matrix'):
            # Create sample confusion matrix
            conf_mat = np.array([[50, 10], [5, 35]])
            
            # Plot
            fig, ax = mngs.plt.subplots()
            if hasattr(mngs.plt, 'confusion_matrix'):
                mngs.plt.confusion_matrix(conf_mat, ax=ax)
            elif hasattr(mngs.plt, 'plot_confusion_matrix'):
                mngs.plt.plot_confusion_matrix(conf_mat, ax=ax)
...
class TestPlotEnhancements:
    ...
    def test_figure_size_control(self):
        ...
        for size in sizes:
            fig, ax = mngs.plt.subplots(figsize=size)
            fig_size = fig.get_size_inches()
            np.testing.assert_array_almost_equal(fig_size, size)
    
    def test_style_consistency(self):
        """Test that mngs maintains consistent styling."""
        # Create multiple figures
        fig1, ax1 = mngs.plt.subplots()
        fig2, ax2 = mngs.plt.subplots()
...
class TestIntegration:
    """Test integration with other mngs modules."""
