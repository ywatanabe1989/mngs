...
__FILE__ = "./tests/mngs/decorators/test___init__.py"
...
def test_decorator_callable():
    ...
    for dec_name in decorators_to_test:
        if hasattr(mngs.decorators, dec_name):
            decorator = getattr(mngs.decorators, dec_name)
...
def test_converters_from_module():
    ...
    for conv in potential_converters:
        if hasattr(mngs.decorators, conv):
            available_converters.append(conv)
            assert callable(getattr(mngs.decorators, conv))
...
def test_data_type_decorators():
    ...
    data_type_items = []
    for attr in dir(mngs.decorators):
...
def test_combined_decorators():
    ...
    for dec in combined_decorators:
        assert callable(getattr(mngs.decorators, dec))
...
def test_preserve_doc_functionality():
    ...
    @mngs.decorators.preserve_doc(original_function)
...
def test_cache_disk_basic():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with patch.dict(os.environ, {'MNGS_DIR': tmpdir + '/'}):
            call_count = 0
            
            @mngs.decorators.cache_disk
...
def test_timeout_decorator_fast_function():
    ...
    @mngs.decorators.timeout(2.0)  # 2 second timeout
...
def test_not_implemented_raises():
    ...
    @mngs.decorators.not_implemented
...
def test_auto_order_enable_disable():
    ...
    with patch('builtins.print') as mock_print:
        mngs.decorators.enable_auto_order()
        mock_print.assert_called()
        assert "Auto-ordering enabled" in str(mock_print.call_args)
    
    # Test disable  
    with patch('builtins.print') as mock_print:
        mngs.decorators.disable_auto_order()
        mock_print.assert_called()
