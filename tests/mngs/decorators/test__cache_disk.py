...
# File: ./mngs_repo/tests/mngs/decorators/test__cache_disk.py
...
class TestCacheDisk:
    """Test cases for mngs.decorators._cache_disk module."""
    ...
    def test_cache_disk_import(self):
        ...
        from mngs.decorators._cache_disk import cache_disk
        assert callable(cache_disk)

    @patch.dict(os.environ, {'MNGS_DIR': ''})
    ...
    def test_cache_disk_uses_mngs_dir_environment_variable(self):
        """Test that cache_disk respects MNGS_DIR environment variable."""
        with tempfile.TemporaryDirectory() as temp_dir:
            custom_mngs_dir = temp_dir + "/custom_mngs/"
            
            with patch.dict(os.environ, {'MNGS_DIR': custom_mngs_dir}):
                from mngs.decorators._cache_disk import cache_disk
                ...
                expected_cache_dir = custom_mngs_dir + "cache/"
    ...
    def test_cache_disk_default_cache_location(self):
        """Test cache_disk uses default location when MNGS_DIR not set."""
        with patch.dict(os.environ, {}, clear=True):
            # Remove MNGS_DIR if it exists
            if 'MNGS_DIR' in os.environ:
                del os.environ['MNGS_DIR']
    ...
    @patch('mngs.decorators._cache_disk._Memory')
    ...
    def test_cache_disk_cache_directory_creation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            custom_mngs_dir = os.path.join(temp_dir, "test_mngs")
    ...
    def test_cache_disk_memory_verbose_setting(self):
        ...
        with patch('mngs.decorators._cache_disk._Memory') as mock_memory:
            from mngs.decorators._cache_disk import cache_disk
