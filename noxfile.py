#!/usr/bin/env python3
"""Nox configuration for MNGS project.
...
def tests(session):
    ...
    # Run pytest
    args = session.posargs or ["tests/", "--cov=mngs", "--cov-report=term-missing", "-v"]
...
def lint(session):
    ...
    # Run bandit
    session.run("bandit", "-r", "src/mngs", "-ll")
...
def typecheck(session):
    ...
    session.run("mypy", "src/mngs", "--ignore-missing-imports")
...
def pylint(session):
    ...
    session.run("pylint", "src/mngs", "--rcfile=pyproject.toml")
...
def security(session):
    ...
    # Run bandit
    session.run("bandit", "-r", "src/mngs", "-ll", "-x", "tests")
