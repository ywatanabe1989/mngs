...
!-- Author: ywatanabe
!-- File: /ssh:ywatanabe@sp:/home/ywatanabe/proj/.claude-worktree/mngs_repo/src/mngs/plt/README.md
!-- --- -->

# SciTeX-Code: Scientific Computing Foundation (formerly mngs.plt)
...
mngs.plt now includes a revolutionary metadata system with clean separation of concerns and YAML export:
...
mngs.io.save(fig, 'dose_response.png')  # YAML metadata automatically saved!
```
...
fig, ((ax1, ax2), (ax3, ax4)) = mngs.plt.subplots(2, 2)
...
mngs.io.save(fig, 'multi_panel_analysis.png')
```
...
# Assert (Save)
mngs.io.save(fig, "basic_line_plot.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "scatter_plot.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "histogram.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "pie_chart.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "errorbar_plot.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "fill_between.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "plot_mean_std.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "plot_mean_ci.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "plot_median_iqr.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "plot_shaded_line.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "plot_raster.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "plot_conf_mat.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "plot_ecdf.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "plot_heatmap.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "plot_violin.gif")
```
...
```python
# Arrange
fig, ax = mngs.plt.subplots(subplot_kw=dict(projection='polar'))
...
# Assert (Save)
mngs.io.save(fig, "plot_circular_hist.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "plot_fillv.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "plot_scatter_hist.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "plot_joyplot.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "sns_barplot.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "sns_boxplot.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "sns_violinplot.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "sns_heatmap.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "sns_scatterplot.gif")
```
...
mngs.str.auto_factor_axis(ax, axis='both', precision=2, min_factor_power=3)
...
# Assert (Save)
mngs.io.save(fig, 'factor_out_digits.png')
```
...
mngs.plt.ax.set_log_scale(ax, axis='both', show_minor_ticks=True, grid=True, minor_grid=True)
...
# Assert (Save)
mngs.io.save(fig, 'log_scale_demo.png')
```
...
formatted_xlabel = mngs.str.format_axis_label('time', unit='seconds', capitalize=True)
formatted_ylabel = mngs.str.format_axis_label('voltage', unit='mV', capitalize=True)
...
# Assert (Save)
mngs.io.save(fig, 'text_formatting.png')
```
...
mngs.plt.ax.show_classic_spines(ax1)  # Bottom and left only
...
mngs.plt.ax.show_all_spines(ax2)  # All four spines
...
mngs.plt.ax.show_spines(ax4, top=False, right=False, spine_width=2.0)
...
# Assert (Save)
mngs.io.save(fig, 'spine_styles.png')
```
...
# Assert (Save)
mngs.io.save(fig, 'enhanced_raster.png')
```
...
mngs.io.save(fig, "set_xytc_example.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "hide_spines.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "set_n_ticks.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "rotate_labels.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "extend_limits.gif")
```
...
# Assert (Save)
mngs.io.save(fig, "legend_positioning.gif")
```
...
# Act
colors = mngs.plt.color.get_colors_from_cmap(n=n_lines, cmap='viridis')
...
# Assert (Save)
mngs.io.save(fig, "colormap_colors.gif")
```
...
# Act
interpolated_colors = mngs.plt.color.interpolate(start_colors, n=10)
...
# Assert (Save)
mngs.io.save(fig, "color_interpolation.gif")
```
...
# Act
mngs.plt.color.vizualize_colors(colors)
...
mngs.io.save(fig, 'export_demo.png')  # Creates PNG, CSV, SigmaPlot CSV, and caption files
```
...
mngs.io.save(fig, 'manual_export.gif')
```
...
# Assert (Save)
mngs.io.save(fig, "multiple_subplots_demo.gif")
```
...
fig, ((ax1, ax2), (ax3, ax4)) = mngs.plt.subplots(2, 2, figsize=(12, 10))
...
mngs.io.save(fig, 'figure_2_comprehensive.png')
```

### Journal-Specific Caption Formats
```python
# Configure caption style for different journals
from mngs.plt.utils._scientific_captions import configure_caption_style
...
from mngs.plt.utils._scientific_captions import export_for_scitex
...
from mngs.plt.utils._scitex_integration import SciTeXManager

# Initialize SciTeX-Paper manager
stm = SciTeXManager('~/proj/SciTeX-Paper/')

# Figures automatically registered during save
mngs.io.save(fig1, 'figure_1.png')  # Auto-registered in SciTeX-Paper
mngs.io.save(fig2, 'figure_2.png')  # Auto-registered in SciTeX-Paper
...
from mngs.plt.utils._scientific_captions import generate_latex_figure
...
from mngs.plt.utils._scientific_captions import ManuScript

ms = ManuScript("research_paper_2024")

# Figures are automatically registered when saved with captions
mngs.io.save(fig1, 'figure_1.png')  # Auto-registered
mngs.io.save(fig2, 'figure_2.png')  # Auto-registered
...
mngs.plt.tpl(x, y)  # Creates ASCII plot in terminal
```
...
### Main Functions
- **`mngs.plt.subplots(*args, track=True, **kwargs)`** - Enhanced subplot creation
- **`mngs.plt.close(fig=None)`** - Close figures
- **`mngs.plt.tpl(x, y)`** - Terminal plotting
...
### Color Utilities
- **`mngs.plt.color.get_colors_from_cmap()`**, **`interpolate()`**, **`vizualize_colors()`**

### Advanced Scientific Features  
- **`mngs.str.auto_factor_axis()`** - Automatic factor-out-of-digits notation
- **`mngs.plt.ax.set_log_scale()`** - Enhanced logarithmic scaling with minor ticks
- **`mngs.str.format_axis_label()`**, **`format_plot_text()`** - Scientific text formatting
- **`mngs.plt.ax.show_classic_spines()`**, **`show_all_spines()`** - Enhanced spine control
...
### The SciTeX Ecosystem: MNGS + SciTeX-Code + SciTeX-Paper + SigMacro
```python
# REVOLUTIONARY: From Raw Data to Published Paper with AI Agents

# 1. DATA ANALYSIS WITH MNGS (The Foundation)
import mngs
import numpy as np

# Load experimental data with automatic preprocessing
data = mngs.io.load('experiment_data.csv')
time = data['time']
signal = data['voltage']
...
mngs.io.save(fig, 'neural_recording.png')  # Triggers the entire ecosystem
...
mngs.io.save(fig, 'experiment.png')  # → Triggers SciTeX ecosystem integration
```

### 🔧 SciTeX Ecosystem Setup
```python
# One-time ecosystem configuration
from mngs.plt.utils._scitex_config import configure_scitex_ecosystem
...
- ✅ **MNGS Foundation** - Scientific data analysis with YAML metadata
...
**Vision Achievement:**
```
Raw Data → MNGS Analysis → YAML Metadata → AI Processing → Publication
    ↓           ↓              ↓              ↓            ↓
