...
def set_meta(ax, caption=None, methods=None, stats=None, keywords=None, 
    ...
    Parameters
    ----------
    ax : matplotlib.axes.Axes or mngs AxisWrapper
        The axes to modify
    ...
    **kwargs : additional metadata
    ...
    ... )
    >>> mngs.io.save(fig, 'neural_recording.png')  # YAML metadata auto-saved
    """
    ...
    metadata['mngs_version'] = '1.11.0'  # TODO: Get from __version__
    
    # Store metadata in figure for automatic saving
    fig = ax.get_figure()
    if not hasattr(fig, '_mngs_metadata'):
        fig._mngs_metadata = {}
    
    # Use axis as key for panel-specific metadata
    fig._mngs_metadata[ax] = metadata
    
    # Also store as YAML-ready structure
    if not hasattr(fig, '_mngs_yaml_metadata'):
        fig._mngs_yaml_metadata = {}
    fig._mngs_yaml_metadata[ax] = metadata
    
    # Backward compatibility - store simple caption
    if caption is not None:
        if not hasattr(fig, '_mngs_captions'):
            fig._mngs_captions = {}
        fig._mngs_captions[ax] = caption
...
def set_figure_meta(ax, caption=None, methods=None, stats=None, significance=None,
    ...
    **kwargs : additional metadata
    ...
    # Store in figure
    fig = ax.get_figure()
    fig._mngs_figure_metadata = figure_metadata
    
    # Backward compatibility
    if caption is not None:
        fig._mngs_main_caption = caption
...
def export_metadata_yaml(fig, filepath):
    ...
    if hasattr(fig, '_mngs_figure_metadata'):
        export_data['figure_metadata'] = fig._mngs_figure_metadata
    
    # Panel-level metadata
    if hasattr(fig, '_mngs_yaml_metadata'):
        for i, (ax, metadata) in enumerate(fig._mngs_yaml_metadata.items()):
