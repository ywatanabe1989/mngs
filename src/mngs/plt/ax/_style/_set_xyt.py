...
def set_xytc(ax, x=False, y=False, t=False, c=False, methods=False, stats=False, format_labels=True):
    ...
    c : str or False, optional
        Caption to store for later use with mngs.io.save(), by default False
    ...
    >>> mngs.io.save(fig, 'neural_signal.png')  # Caption automatically saved
    """
    # Set labels and title using existing function
    set_xyt(ax, x=x, y=y, t=t, format_labels=format_labels)
    
    # Store caption and extended metadata for later use by mngs.io.save()
    if c is not False or methods is not False or stats is not False:
        ...
        if hasattr(ax, '_mngs_metadata'):
            ax._mngs_metadata.update(metadata)
        else:
        ...
        if c is not False:
            if hasattr(ax, '_mngs_caption'):
                ax._mngs_caption = c
            else:
                fig = ax.get_figure()
                if not hasattr(fig, '_mngs_captions'):
                    fig._mngs_captions = {}
                fig._mngs_captions[ax] = c
