...
**Old test file** (`tests/mngs/ai/_gen_ai/test__genai_factory.py`):
```python
def main(
...
**New example** (`examples/mngs/ai/genai_example.py`):
```python
#!/usr/bin/env python3
...
```python
# src/mngs/ai/_gen_ai/_genai_factory.py (compatibility wrapper)
import warnings
...
def genai_factory(model_or_provider, api_key=None, **kwargs):
    """
    Deprecated: Use mngs.ai.genai.GenAI instead.
    
    This function is provided for backward compatibility only.
    """
    warnings.warn(
        "genai_factory is deprecated. Use mngs.ai.genai.GenAI instead.",
        DeprecationWarning,
        stacklevel=2
    )
...
1. Import from `mngs.ai.genai` instead of `mngs.ai._gen_ai`
