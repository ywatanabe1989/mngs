...
test:
	python -m pytest tests/ -v --cov=src/mngs --cov-report=html --cov-report=term-missing --cov-fail-under=85

test-fast:
	python -m pytest tests/ -v -n auto --cov=src/mngs --cov-report=term

test-module:
	@echo "Usage: make test-module MODULE=gen"
	python -m pytest tests/mngs/$(MODULE) -v --cov=src/mngs/$(MODULE) --cov-report=term-missing

test-slow:
	python -m pytest tests/ -v --cov=src/mngs -m "slow"

test-unit:
	python -m pytest tests/ -v --cov=src/mngs -m "not integration" --cov-report=term-missing

test-integration:
	python -m pytest tests/ -v --cov=src/mngs -m "integration" --cov-report=term-missing

coverage:
	python -m pytest tests/ --cov=src/mngs --cov-report=html --cov-report=term-missing --cov-report=xml
...
lint:
	@echo "Running flake8..."
	flake8 src/mngs --max-line-length=88 --extend-ignore=E203,W503,E731
...
typecheck:
	mypy src/mngs --ignore-missing-imports --show-error-codes
...
# CI/CD helpers
ci-test:
	python -m pytest tests/ --cov=src/mngs --cov-report=xml --cov-fail-under=85
...
quality-report:
	...
	@mkdir -p reports
	flake8 src/mngs --format=html --htmldir=reports/flake8
	mypy src/mngs --html-report reports/mypy
	bandit -r src/mngs -f html -o reports/bandit.html
...
# Docker support
docker-build:
	docker build -t mngs:latest .

docker-test:
	docker run --rm mngs:latest pytest

docker-shell:
	docker run --rm -it -v $$(pwd):/app mngs:latest bash