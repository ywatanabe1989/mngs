#!/usr/bin/env python3
"""
Module dependency analyzer for MNGS framework.
...
class ModuleDependencyAnalyzer:
    def __init__(self, project_root: str):
        ...
        self.src_root = self.project_root / "src" / "mngs"
        self.dependencies: Dict[str, Set[str]] = defaultdict(set)
        self.internal_modules = self._get_internal_modules()
        
    def _get_internal_modules(self) -> Set[str]:
        """Get all internal MNGS modules."""
        modules = set()
        # Add top-level modules
        for dir_path in self.src_root.iterdir():
            if dir_path.is_dir() and not dir_path.name.startswith("_") and not dir_path.name.startswith("."):
                modules.add(f"mngs.{dir_path.name}")
        ...
        for py_file in self.src_root.rglob("*.py"):
            ...
            if py_file.name == "__init__.py":
                ...
                if str(rel_path) != ".":
                    ...
                    modules.add(f"mngs.{module_path}")
            else:
        ...
        for py_file in self.src_root.rglob("*.py"):
            ...
            if py_file.name == "__init__.py":
                if str(rel_path.parent) == ".":
                    ...
                    module_name = "mngs." + str(rel_path.parent).replace("/", ".")
            else:
                module_name = "mngs." + str(rel_path).replace("/", ".").replace(".py", "")
            
            # Extract imports
            imports = self._extract_imports(py_file)
            
            # Filter for internal MNGS imports
            for imp in imports:
                if imp.startswith("mngs"):
                                ...
                                if node.level > 0:  # Relative import
                                    ...
                                    if node.level <= len(parts) - 1:
                                        ...
                                        if node.module:
                                            ...
                                            target = base
                                        if target.startswith("mngs") and target != module_name:
    ...
    def get_module_statistics(self) -> Dict[str, Dict]:
        ...
        top_level_modules = defaultdict(lambda: {"files": 0, "deps_in": set(), "deps_out": set()})
        
        for module, deps in self.dependencies.items():
            # Extract top-level module (e.g., "mngs.io" from "mngs.io._load")
            parts = module.split(".")
            if len(parts) >= 2:
    ...
    def visualize_dependencies(self, output_path: str = "module_dependencies.png"):
        ...
        for module, deps in self.dependencies.items():
            ...
            parts = module.split(".")
            if len(parts) >= 2:
                top_level = f"{parts[1]}"  # Just the module name without "mngs."
        ...
        plt.title("MNGS Module Dependencies", fontsize=16, fontweight='bold')
        plt.axis('off')
        plt.tight_layout()
    ...
    def generate_report(self) -> str:
        ...
        report = []
        report.append("# MNGS Module Dependency Analysis Report\n")
        
        # Summary
        stats = self.get_module_statistics()
        report.append("## Summary")
        report.append(f"- Total modules analyzed: {len(stats)}")
        report.append(f"- Total dependencies: {sum(s['coupling'] for s in stats.values()) // 2}")
        ...
        # Module statistics
        report.append(f"\n## Module Statistics")
        report.append("| Module | Files | Dependencies In | Dependencies Out | Total Coupling |")
        report.append("|--------|-------|-----------------|------------------|----------------|")
        
        sorted_modules = sorted(stats.items(), key=lambda x: x[1]['coupling'], reverse=True)
        for module, info in sorted_modules:
            module_name = module.replace("mngs.", "")
            report.append(f"| {module_name} | {info['files']} | {info['dependencies_in']} | {info['dependencies_out']} | {info['coupling']} |")
        ...
        for module, info in sorted_modules:
            if info['coupling'] > 0:
                ...
                if info['deps_in_list']:
                    report.append(f"**Depends on by:** {', '.join(d.replace('mngs.', '') for d in info['deps_in_list'])}")
                if info['deps_out_list']:
                    report.append(f"**Depends on:** {', '.join(d.replace('mngs.', '') for d in info['deps_out_list'])}")
        ...
        if high_coupling:
            ...
            report.append("These modules have high coupling and might benefit from refactoring:")
...
if __name__ == "__main__":
    # Run analysis
    analyzer = ModuleDependencyAnalyzer("/data/gpfs/projects/punim2354/ywatanabe/.claude-worktree/mngs_repo")
