    ...
    def analyze_dependencies(self):
        ...
        for py_file in self.src_root.rglob("*.py"):
            ...
            if py_file.name == "__init__.py":
                ...
                module_name = "mngs." + str(rel_path).replace("/", ".").replace(
                    ".py", ""
                )
        ...
        for module, info in sorted_modules:
            if info["coupling"] > 0:
                ...
                if info["deps_in_list"]:
                    report.append(
                        f"**Depends on by:** {', '.join(d.replace('mngs.', '') for d in info['deps_in_list'])}"
                    )
                if info["deps_out_list"]:
                    report.append(
                        f"**Depends on:** {', '.join(d.replace('mngs.', '') for d in info['deps_out_list'])}"
                    )
...
if __name__ == "__main__":
    ...
    analyzer = ModuleDependencyAnalyzer(
        "/data/gpfs/projects/punim2354/ywatanabe/.claude-worktree/mngs_repo"
    )
