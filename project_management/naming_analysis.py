#!/usr/bin/env python3
"""
Analyze naming inconsistencies in the MNGS codebase.
...
class NamingAnalyzer:
    ...
    def check_file_naming(self) -> Dict[str, List[str]]:
        ...
        for py_file in self.src_root.rglob("*.py"):
            ...
            if any(suffix in filename for suffix in ["_v0", "_v1", "_dev", "_old", "_backup"]):
                issues.append(f"Version suffix: {py_file.relative_to(self.project_root)}")
                
            # Check for camelCase files (should be snake_case)
            if not self.snake_case_pattern.match(filename) and not filename.startswith("_"):
                if any(c.isupper() for c in filename[1:]):  # Has uppercase after first char
                    issues.append(f"Not snake_case: {py_file.relative_to(self.project_root)}")
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ClassDef):
                        ...
                        if not self.pascal_case_pattern.match(class_name):
                            ...
                            if not (class_name.startswith("_") and self.pascal_case_pattern.match(class_name[1:])):
                                rel_path = py_file.relative_to(self.project_root)
                                issues.append(f"{rel_path}:{node.lineno} - {class_name}")
                ...
                for i, line in enumerate(lines, 1):
                    for pattern, preferred in abbrev_patterns:
                        if re.search(pattern, line):
                            rel_path = py_file.relative_to(self.project_root)
                            issues.append(f"{rel_path}:{i} - '{pattern}' should be '{preferred}'")
    ...
    def generate_report(self) -> str:
        ...
        report = ["# MNGS Naming Inconsistencies Analysis\n"]
        
        # File naming
        file_issues = self.check_file_naming()
        report.append("## File Naming Issues")
        if file_issues["file_naming"]:
            report.append(f"Found {len(file_issues['file_naming'])} file naming issues:\n")
        ...
        if func_issues["function_naming"]:
            report.append(f"Found {len(func_issues['function_naming'])} functions not following snake_case:\n")
            for issue in func_issues["function_naming"][:10]:
                report.append(f"- {issue}")
            if len(func_issues["function_naming"]) > 10:
                report.append(f"... and {len(func_issues['function_naming']) - 10} more")
        else:
        ...
        if class_issues["class_naming"]:
            report.append(f"Found {len(class_issues['class_naming'])} classes not following PascalCase:\n")
        ...
        if doc_issues["missing_docstrings"]:
            report.append(f"Found functions/classes without docstrings (showing first 20):\n")
        ...
        report.append(f"- Function naming issues: {len(func_issues['function_naming'])}")
...
if __name__ == "__main__":
    analyzer = NamingAnalyzer("/data/gpfs/projects/punim2354/ywatanabe/.claude-worktree/mngs_repo")
