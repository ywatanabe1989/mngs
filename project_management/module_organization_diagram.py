#!/usr/bin/env python3
"""
Generate module organization diagrams for MNGS.
...
def create_current_organization():
    ...
    # Title
    ax.text(7, 9.5, 'MNGS Current Module Organization', 
    ...
    module_sizes = {
        'ai': 40, 'plt': 31, 'dsp': 25, 'db': 20, 'stats': 17,
        'nn': 15, 'gen': 13, 'decorators': 10, 'io': 10,
        'str': 5, 'path': 4, 'pd': 4, 'resource': 4, 'linalg': 3,
        'utils': 3, 'dev': 2, 'web': 2, 'torch': 1, 'types': 1,
        'dict': 1, 'context': 1, 'dt': 1, 'os': 1, 'parallel': 1,
        'tex': 1, 'gists': 1, 'reproduce': 1, 'etc': 1, 'life': 1
    }
    ...
    for category, info in categories.items():
        ...
        # Category box
        cat_box = FancyBboxPatch((x-1.3, y-1.8), 2.6, 2.3,
        ...
        for i, module in enumerate(modules):
            ...
            ax.text(x, y + y_offset, f"{module} ({size})", 
                   fontsize=9, ha='center', color=text_color, weight=weight)
    ...
    # Add issues box
    issues_box = FancyBboxPatch((0.5, 0.2), 6, 0.8,
                                boxstyle="round,pad=0.05",
                                facecolor='#FFCCCC',
                                edgecolor='red',
                                linewidth=1)
    ax.add_patch(issues_box)
    ax.text(3.5, 0.6, 'Issues: 107 total (7 temp files, 5 versioned, 30 duplicates, 2 vendored)', 
...
def create_proposed_organization():
    ...
    # Title
    ax.text(7, 9.5, 'MNGS Proposed Module Organization', 
            fontsize=20, fontweight='bold', ha='center')
    ...
    for key, info in proposed.items():
        ...
        ax.text(x, y+0.1, f"({info['desc']})", fontsize=8, ha='center', style='italic')
    ...
    ai_box = FancyBboxPatch((9.7, 5.7), 2.6, 1.5,
                           boxstyle="round,pad=0.05",
                           facecolor='#FFFFCC',
                           edgecolor='orange',
                           linewidth=1,
    ...
    imp_box = FancyBboxPatch((0.5, 0.2), 8, 1,
                            boxstyle="round,pad=0.05",
                            facecolor='#CCFFCC',
                            edgecolor='green',
                            linewidth=1)
    ...
    ax.text(4.5, 0.5, '✓ Clear module boundaries  ✓ No duplicates  ✓ Better organization  ✓ Reduced coupling', 
           fontsize=9, ha='center')
    
    # Add arrows showing consolidation
    arrow1 = ConnectionPatch((7, 3.5), (8, 4), "data", "data",
                            arrowstyle="->", shrinkA=5, shrinkB=5,
                            mutation_scale=20, fc="gray", alpha=0.5)
...
def create_module_coupling_heatmap():
    ...
    modules = ['io', 'decorators', 'nn', 'dsp', 'gen', 'plt', 'stats', 
               'str', 'types', 'dict', 'utils', 'ai', 'linalg']
    coupling = [28, 22, 20, 19, 15, 11, 10, 7, 7, 5, 4, 4, 4]
    
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
    
    # Create bar chart
    colors = ['red' if c > 20 else 'orange' if c > 10 else 'yellow' if c > 5 else 'green' for c in coupling]
    ...
    ax.axhline(y=10, color='orange', linestyle='--', alpha=0.5, label='High coupling threshold')
    ax.axhline(y=20, color='red', linestyle='--', alpha=0.5, label='Very high coupling threshold')
